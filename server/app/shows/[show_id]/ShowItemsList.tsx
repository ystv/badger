...
import { addItem, deleteItem, reorderShowItems } from "./actions";
...
export function ShowItemsList(props: {
  ...
  const rows = useMemo(() => {
        ...
        >
          {(provided, snapshot) => (
            <tr
              ...
              data-testid={item._type + ":" + item.id}
            ...
            >
              ...
              <td
                className={clsx(
                  "font-bold",
                  item._type === "rundown" ? "text-primary" : "text-purple"
                )}
              >
                {item._type}
              </td>
              <td>{item.name}</td>
              <td>{format(time, "HH:mm")}</td>
              <td>{format(new Date(duration * 1000), "mm:ss")}</td>
              <td>
                {item._type === "rundown" ? (
                  <Link href={`/shows/${props.show.id}/rundown/${item.id}`}>
    ...
    return rows;
  }, [optimisticItems, isPending, props.show.id, props.show.start]);
